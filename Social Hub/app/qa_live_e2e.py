"""Seeded end-to-end QA validation for SocialHub against live Supabase test schema."""
...
HOST = os.getenv("QA_LIVE_E2E_HOST", "127.0.0.1")
PORT = int(os.getenv("QA_LIVE_E2E_PORT", "8016"))
BASE_URL = os.getenv("QA_LIVE_E2E_BASE_URL", f"http://{HOST}:{PORT}")
...
def location_matches(location: str | None, expected_suffix: str) -> bool:
    if not location:
        return False
    return location == expected_suffix or location.endswith(expected_suffix)


def to_internal_path(location: str) -> str:
    if location.startswith("http://") or location.startswith("https://"):
        from urllib.parse import urlparse
        parsed = urlparse(location)
        location = parsed.path or "/"
    if location.startswith("/social-hub/"):
        return location[len("/social-hub"):]
    if location == "/social-hub":
        return "/"
    return location


def wait_for_server(client: httpx.Client, timeout_seconds: int = 30):
...
def run_checks():
    if not settings.USES_SUPABASE_DATABASE:
        raise SystemExit("Live E2E QA requires SUPABASE_DB_URL or DATABASE_URL to point to Supabase Postgres.")
    if settings.APP_ENV != "test" or settings.DATABASE_SCHEMA != "test":
        raise SystemExit(
            "Live seeded E2E QA is restricted to the shared test schema. Set APP_ENV=test and DATABASE_SCHEMA=test before running it."
        )
    ...
    try:
        with httpx.Client(base_url=BASE_URL, follow_redirects=False, timeout=20.0) as client:
            ...
            pages = [
                ("/", ["SocialHub", "Dashboard", "LinkedIn", "Instagram", "Published", "Draft"], []),
            ...
            assert_true(health_json.get("database") == "ok", "health reports database ok")
            assert_true(health_json.get("database_target") == "supabase", "health reports Supabase target")
            assert_true(health_json.get("database_schema") == settings.DATABASE_SCHEMA, "health reports configured schema")

            supabase_public_api = health_json.get("supabase_public_api", {})
            assert_true(supabase_public_api.get("status") == "ok", "health reports Supabase public API ok")

            csv_export = client.get("/api/posts/export?format=csv")
            assert_true(csv_export.status_code == 200, "CSV export returned 200")
            assert_true(
                "platform" in csv_export.text and "linkedin" in csv_export.text and "instagram" in csv_export.text,
                "CSV export includes both platform values",
            )
            ...
            topic_name = "QA Topic: Supabase live seeded flow"
            ...
            assert_true(location_matches(add_topic.headers.get("location"), "/topics"), "topic add redirected to /topics")
            ...
            assert_true("/posts/" in duplicate_location, "duplicate redirect points to new post")
            duplicate_detail = client.get(to_internal_path(duplicate_location))
            assert_true("[Copy] IT Skills Shortage in 2026" in duplicate_detail.text, "duplicate post detail renders new copy")

            duplicate_post_id = int(to_internal_path(duplicate_location).rsplit("/", 1)[-1])
        ...
        print("\nLIVE SUPABASE SEEDED END-TO-END QA PASSED")
    finally:
        stop_server(server)
...
if __name__ == "__main__":
    try:
        ...
        print(f"LIVE SUPABASE SEEDED QA FAILED: {exc}")
        sys.exit(1)