/* ==============================================
   Momentum Landing Page — JS (integrated version)
   Vanilla JS, scoped to .landing-page container
   ============================================== */
(function () {
    'use strict';
    var root = document.querySelector('.landing-page');
    if (!root) return;
    ...
    function initRoadmap() {
        ...
        var rows = Array.prototype.slice.call(timeline.querySelectorAll('.rm__row'));
    ...
    function initParticles() {
        ...
        function create() {
            ...
            return {
                ...
                hue: Math.random() < .6 ? Math.random() * 40 : 40 + Math.random() * 40
            };
        }
        ...
        function animate() {
            ...
            for (var i = 0; i < particles.length; i++) {
                ...
                if (h < 40) {
                    var t = h / 40;
                    r = Math.round(193 + t * 46);
                    g = Math.round(41  + t * 27);
                    b = Math.round(46  + t * 22);
                } else {
                    var t2 = (h - 40) / 40;
                    r = Math.round(239 - t2 * 148);
                    g = Math.round(68  + t2 * 24);
                    b = Math.round(68  + t2 * 180);
                }
        ...
        hero.addEventListener('mousemove', function (e) { var rect = canvas.getBoundingClientRect(); mouse.x = e.clientX - rect.left; mouse.y = e.clientY - rect.top; });
    ...
    function initMobileMenu() {
        ...
        Array.prototype.slice.call(menu.querySelectorAll('.mm-link')).forEach(function (l) { l.addEventListener('click', close); });
    ...
    function initCookie() {
        ...
        if (!banner || localStorage.getItem('momentum_cookie')) return;
        setTimeout(function () { banner.classList.add('cookie-banner--visible'); }, 2000);
        Array.prototype.slice.call(banner.querySelectorAll('[data-cookie-action]')).forEach(function (btn) {
            btn.addEventListener('click', function () {
                localStorage.setItem('momentum_cookie', btn.dataset.cookieAction);
