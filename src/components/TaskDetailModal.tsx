...
export default function TaskDetailModal({
  ...
  const hasUnsavedEdits =
    isEditing && JSON.stringify(editedTask) !== JSON.stringify(task);

  const requestClose = () => {
    if (
      hasUnsavedEdits &&
      !window.confirm(
        "Es gibt ungespeicherte Änderungen. Möchtest du das Modal wirklich schließen?",
      )
    ) {
      return;
    }
  ...
  useEffect(() => {
    const campaign = task.campaignId
      ? campaigns.find((c) => c.id === task.campaignId)
      : null;
    const audience = campaign?.targetAudiences?.[0]
      ? audiences.find((a) => a.id === campaign.targetAudiences[0])
      : audiences[0] || null;
    const touchpoint = task.touchpointId
      ? touchpoints.find((tp) => tp.id === task.touchpointId)
      : null;
  ...
  const isEligibleSocialTask =
    (task.platform === "LinkedIn" || task.platform === "Instagram") &&
    [
      "Post (Beschreibung)",
      "Post (Foto)",
      "Videoskript",
  ...
  useEffect(() => {
    ...
    listPosts({ companyId: activeCompany.id, limit: 500 })
      .then((posts) => {
        if (!cancelled)
          setLinkedSocialPosts(posts.filter((p) => p.task_id === task.id));
      })
      .catch(() => {
        /* SH offline — task modal still works */
      });
    return () => {
      cancelled = true;
    };
        ...
        >
          ...
          >
            ...
            >
              ...
              {isEditing ? (
                ...
                >
                  <option value="Post (Beschreibung)">
                    Post (Beschreibung)
                  </option>
          ...
          <div className="card" style={{ marginBottom: "16px" }}>
            ...
            >
              ...
              >
                <User size={14} /> {task.author || "System"}
              </div>
              ...
              {isEditing ? (
                ...
                <div style={{ fontWeight: 500, color: "var(--color-primary)" }}>
                  {getTouchpointName(task.touchpointId)}
                </div>
              )}
              ...
              {isEditing ? (
                ...
                >
                  <Clock size={14} />
                  {task.publishDate
                    ? new Date(task.publishDate).toLocaleString("de-DE", {
                        dateStyle: "short",
                        timeStyle: "short",
                      })
                    : "Noch nicht gesetzt"}
          ...
          {linkedContents.length > 0 && (
            <div
              className="card"
              style={{ marginBottom: "16px", borderLeft: "4px solid #8b5cf6" }}
            >
              <h4
                style={{
                  fontSize: "var(--font-size-sm)",
              ...
              >
                <FileText size={16} style={{ color: "#8b5cf6" }} /> Zugehöriger
                Content
              </h4>
              <div
                    ...
                    >
                      <div>
                        ...
                        >
                          ...
                          <span>
                            {cnt.publishDate
                              ? new Date(cnt.publishDate).toLocaleDateString(
                                  "de-DE",
                                )
                              : "Kein Datum"}
                          </span>
        ...
        >
          {isEditing ? (
            ...
            <div style={{ display: "flex", gap: "8px", width: "100%" }}>
              <Link
                href={companyPath(`/campaigns/${task.campaignId}`)}
