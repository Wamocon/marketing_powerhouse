...
export async function notifyTaskAssigned(
  ...
  try {
    await api.createNotification({
      ...
      actionUrl: `/project/${opts.companyId}/tasks`,
  ...
  for (const recipientId of opts.recipientUserIds) {
    ...
    try {
      await api.createNotification({
        ...
        actionUrl: `/project/${opts.companyId}/campaigns/${opts.campaignId}`,
        triggeredByUserId: opts.triggeredByUserId,
      });
    } catch (err) {
  ...
  for (const recipientId of opts.recipientUserIds) {
    try {
      await api.createNotification({
        ...
        entityType: 'campaign',
        entityId: opts.campaignId,
        actionUrl: `/project/${opts.companyId}/budget`,
      });
    } catch (err) {
  ...
  for (const recipientId of opts.recipientUserIds) {
    ...
    try {
      await api.createNotification({
        ...
        actionUrl: `/project/${opts.companyId}/content`,
        triggeredByUserId: opts.triggeredByUserId,
      });
    } catch (err) {
  ...
  for (const recipientId of opts.recipientUserIds) {
    ...
    try {
      await api.createNotification({
        ...
        actionUrl: `/project/${opts.companyId}/settings`,
        triggeredByUserId: opts.triggeredByUserId,
      });
    } catch (err) {
